...
def advent_1b(input):
	...
	while True:
		input[0] = first + start
		output = np.cumsum(input)
		# print(output)
		
		if idx == 0:
			output_full	= output
		else:
			output_full	= np.concatenate((output_full, output))
			
		unique, index = np.unique(output_full, return_index=True)
		unique_bin = np.zeros_like(output_full, dtype=bool) # False
		unique_bin[index] = True
		result = output_full[~unique_bin]
