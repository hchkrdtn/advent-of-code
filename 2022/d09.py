...
def get_bbox_direction(input, coord_orient, origin):
    ...
    Args:
        input (list): Points defined by moving direction and distance, snake-like
...
def direct_to_coord(input, coord_orient):
    """ Convert points directions, such as points moving
    in some snake motion to coordinates. Coordinate orientation is also set. Returns t lists of x, y values
...
def advent_a(arr):
    bound = get_bbox_direction(arr, True, False)
    print(bound)
    bound = direct_to_coord(arr, True)
    print(bound)
    bound = get_bbox_coord(bound)
    print(bound)

    # rope = np.array(arr).astype(np.int32)
    # print(rope)
    # rx = rope.shape[0]
    # ry = rope.shape[1]
    # print(rx, ry)
